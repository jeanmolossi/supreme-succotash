...
export default function Outcome({ transaction }: OutcomeProps) {
		...
		<div className="border rounded p-2 grid animate-slide-up-fade [animation-delay:250ms] [animation-duration:1s] [animation-fill-mode:both]">
			...
			<div className="grid grid-cols-2 items-baseline">
				<div className="grid gap-2">
					...
					<small className="text-xs text-muted-foreground">Categoria {transaction.category.name} via {transaction.bank_account.name}</small>
				</div>

				<div className="grid place-items-end gap-2">
					<Link
						href={`/transaction?id=${transaction.id}`}
						className={'text-destructive font-medium text-right'}
					>
					...
					<span className="text-xs uppercase font-semibold text-muted-foreground">Débito/Pix</span>
				</div>
			</div>
		</div >
	)
